class Flower{
  ...
  //decay rates
  float poppyDRate = 0.000005;
  float roseDRate = 0.00000005;
  float bbDRate = 0.00000000000000000005;
  float sunnyDRate = 0.00005;

  Flower(){
  }
  
  //function to draw the seed (circle) using points
  void sketch(float xcenter, float ycenter){
    xloc = xcenter;
    yloc = ycenter;
 ...
 */
 ...
 for(int i = 0;i<Pnum;i++){//poppies
   if(Pnum>0 && Rnum>20){ 
       allpoppies.get(i).poppyDRate = 0.005;
   }else{
      allpoppies.get(i).poppyDRate = 0.000005;
   }
 }
 for(int i = 0;i<Bnum;i++){//baby's breath
   if(Bnum>0 && Rnum>20){
       allbbs.get(i).bbDRate = 0.05;
   }else{
      allbbs.get(i).poppyDRate = 0.00000000000000000005;
   }
 }
